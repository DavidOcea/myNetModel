...
__all__ = ['efficientnet_b0_9','efficientnet_b0','efficientnet_b1','efficientnet_b2','efficientnet_b3']
########################################################################
...
def efficientnet_params(model_name,sets):
    """ Map EfficientNet model name to parameter coefficients. """
    params_dict = {
        # Coefficients:   width,depth,res,dropout
        'efficientnet-b0': (1.0, 1.0, 224, 0.2),  #1.0, 1.0, 224, 0.2
...
def get_model_params(model_name, override_params, num_classes=1000, drop_out=0.2, sets=None):
    """ Get the block args and global params for a given model """
    if model_name.startswith('efficientnet'):
        w, d, _, p = efficientnet_params(model_name, sets)
...
class EfficientNet(nn.Module):
    ...
    @classmethod
    def from_name(cls, model_name, override_params=None, num_classes=1000, drop_out=0.2, sets=None):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(model_name, override_params, num_classes=num_classes, drop_out=drop_out,sets=sets)
    ...
    def _check_model_name_is_valid(cls, model_name, also_need_pretrained_weights=False):
        ...
        if model_name.replace('-','_') not in valid_models:
            print('model_name should be one of: ' + ', '.join(valid_models))
...
def efficientnet_b0(**kwargs):
    ...
    model = EfficientNet.from_name('efficientnet-b0', num_classes=num_classes, drop_out=drop_out, sets=sets)

    return model


def efficientnet_b0_9(**kwargs):
    num_classes = 1000
    drop_out = 0
    sets = (0.5, 0.5, 112, 0.5)
    if 'num_classes' in kwargs:
        num_classes = kwargs['num_classes']
    if 'dropout' in kwargs:
        drop_out = kwargs['dropout']
    if 'sets' in kwargs:
        sets = kwargs['sets']
    model = EfficientNet.from_name('efficientnet-b0_9', num_classes=num_classes, drop_out=drop_out, sets=sets)
